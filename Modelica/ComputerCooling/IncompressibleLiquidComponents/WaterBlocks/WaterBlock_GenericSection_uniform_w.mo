...
model WaterBlock_GenericSection_uniform_w
  ComputerCooling.IncompressibleLiquidComponents.Ducts.FluidStream.LiquidStream_FiniteVolume_GenericSection_uniform_w channels[m](redeclare each model medium = medium, redeclare each model HTCoefficient = HTCoefficient, each L = L, each n = n, each Ac = Ac / m, each per = per / m, each TStart = TStart, each dp_nom = dp_nom, each fluidHeats = false, each w_nom = w_nom / m) annotation(
