...
model Tube_1D_GenericSection
  extends IncompressibleLiquidComponents.BaseClasses.TwoPorts_pwh_OnePort_VHP(massStorage = true);
  
  ComputerCooling.SolidComponents.StreamConfinement.PlanarWall_FiniteVolume wall(L = L, TStart = TStart, W = W, n = n, t = t, m = m) annotation(
    Placement(visible = true, transformation(origin = {0, 54}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));

  
  ComputerCooling.IncompressibleLiquidComponents.Ducts.LiquidStream_FiniteVolume_GenericSection liquidStream(L = L, Ac = Ac, per = per, w_nom = w_nom, dp_nom = dp_nom, TStart = TStart, n = n, fluidHeats = fluidHeats, dz=dz, gamma_corr=gamma_corr,
  ...
  parameter Area               Ac         = Modelica.Constants.pi * (D/2)^2 "stream cross area";
  parameter Length             per        = Modelica.Constants.pi * D "stream cross area perimeter";
  ...
  parameter Integer            m          = 5 "Number of layers, 1 internal <---> m external";
