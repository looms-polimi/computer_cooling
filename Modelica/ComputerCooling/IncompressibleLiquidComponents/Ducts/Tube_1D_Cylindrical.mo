...
model Tube_1D_Cylindrical
  extends IncompressibleLiquidComponents.BaseClasses.TwoPorts_pwh_OnePort_VHP(massStorage = true);

  ComputerCooling.SolidComponents.StreamConfinement.CylindricalWall_FiniteVolume wall( 
  redeclare replaceable record material = materialRecord, L = L, TStart = TStart, D = D, n = n, m = m, t = t) annotation(
  ...
  parameter Real               gamma_corr = 1 "multiplicative correction on internal ccht";
...
equation
  ...
  connect(wall.hp_in, liquidStream.surf) annotation(
    Line(points = {{0, 36}, {0, 24}}, color = {191, 0, 0}));
  connect(wall.hp_ext, surf) annotation(
    Line(points = {{0, 68}, {0, 90}}, color = {191, 0, 0}));
