...
model Tube_2D
  ...
  ComputerCooling.HeatTransfer.StreamSeparators.TubeWall_FiniteVolume_MultiL wall_multiL(L = L, W = W, t = t, TStart = TStart, n = n, l = l, redeclare replaceable record materialRecord = materialRecord) annotation(
    Placement(visible = true, transformation(origin = {0, 20}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
  ComputerCooling.IncompressibleLiquidComponents.Ducts.FluidStream.LiquidStream_FiniteVolume_Cylindrical liquidStream(D = D, L = L, dz = dz, w_nom = w_nom, dp_nom = dp_nom, TStart = TStart, n = n, fluidHeats = fluidHeats, redeclare replaceable model medium = medium, redeclare replaceable model HTCoefficient = HTCoefficient) annotation(
  ...
  parameter Integer l = 5 "Number of layers";
  // 1 innermost <---> l outermost
