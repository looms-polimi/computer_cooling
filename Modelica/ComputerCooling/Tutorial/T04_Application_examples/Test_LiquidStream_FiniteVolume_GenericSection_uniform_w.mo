...
model Test_LiquidStream_FiniteVolume_GenericSection_uniform_w
  ...
  ComputerCooling.IncompressibleLiquidComponents.Ducts.FluidStream.LiquidStream_FiniteVolume_GenericSection_uniform_w lstream(dp_nom = 20000, n = 5, w_nom = 0.5 / 60)  annotation(
  ...
  ComputerCooling.HeatTransfer.BoundaryConditions.Prescribed_Power_1D_uniform hsp(n = 5)  annotation(
...
equation
  ...
  connect(lstream.pwh_b, snk.pwh_a) annotation(
    Line(points = {{14, -10}, {38, -10}}));
  connect(hsp.hp, lstream.surf) annotation(
    Line(points = {{-38, 30}, {2, 30}, {2, -16}}, color = {191, 0, 0}));
