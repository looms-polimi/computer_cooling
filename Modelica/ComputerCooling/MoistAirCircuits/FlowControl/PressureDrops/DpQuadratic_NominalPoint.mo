within ComputerCooling.MoistAirCircuits.FlowControl.PressureDrops;

model DpQuadratic_NominalPoint
  extends BaseClasses.TwoPort_pwhx;
  replaceable model medium = Media.MoistAir;
  medium air;
  
  parameter PressureDifference dp_nom = 1000;
  parameter MassFlowRate q_nom = 0.1;
  
equation
  air.p = pwhx_a.p;
  air.h = pwhx_a.h;
  air.x = pwhx_a.x;

  0 = wa + wb;  //no mass storage
  wa = ComputerCooling.Functions.sqrtReg(((q_nom * air.d)^2/dp_nom) * dp); //flow directed by nominal point
  
  hob = hia;  //no change in enthalpy
  hoa = hib;
  xob = xia;  //no change in mass fraction
  xoa = xib;
  annotation(
    Icon(graphics = {Bitmap(extent = {{-100, -20}, {100, 20}}, imageSource = ""), Text(origin = {1, 48}, extent = {{-87, 46}, {87, -46}}, textString = "quadratic")}));
end DpQuadratic_NominalPoint;
