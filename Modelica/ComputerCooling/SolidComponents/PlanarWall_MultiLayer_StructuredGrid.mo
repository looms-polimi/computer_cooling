...
//edge lumps on first thickness layer
  C[1, 1, 1] * der(T[1, 1, 1]) = hp_in.port[1, 1].Q_flow - G_t[2, 1, 1] * (T[1, 1, 1] - T[1, 1, 2]) - G_w[2, 1, 1] * (T[1, 1, 1] - T[2, 1, 1]) - G_d[2, 1, 1] * (T[1, 1, 1] - T[1, 2, 1]);
  C[w, 1, 1] * der(T[w, 1, 1]) = hp_in.port[w, 1].Q_flow - G_t[2, w, 1] * (T[w, 1, 1] - T[w, 1, 2]) - G_w[w, 1, 1] * (T[w, 1, 1] - T[w - 1, 1, 1]) - G_d[2, w, 1] * (T[w, 1, 1] - T[w, 2, 1]);
  C[1, d, 1] * der(T[1, d, 1]) = hp_in.port[1, d].Q_flow - G_t[2, 1, d] * (T[1, d, 1] - T[1, d, 2]) - G_w[2, d, 1] * (T[1, d, 1] - T[2, d, 1]) - G_d[d, 1, 1] * (T[1, d, 1] - T[1, d - 1, 1]);
  C[w, d, 1] * der(T[w, d, 1]) = hp_in.port[w, d].Q_flow - G_t[2, w, d] * (T[w, d, 1] - T[w, d, 2]) - G_w[w, d, 1] * (T[w, d, 1] - T[w - 1, d, 1]) - G_d[d, w, 1] * (T[w, d, 1] - T[w, d - 1, 1]);
...
//edge lumps on last thickness layer
  C[1, 1, t] * der(T[1, 1, t]) = G_t[t, 1, 1] * (T[1, 1, t - 1] - T[1, 1, t]) + hp_ext.port[1, 1].Q_flow - G_w[2, 1, t] * (T[1, 1, t] - T[2, 1, t]) - G_d[2, 1, t] * (T[1, 1, t] - T[1, 2, t]);
  C[w, 1, t] * der(T[w, 1, t]) = G_t[t, w, 1] * (T[w, 1, t - 1] - T[w, 1, t]) + hp_ext.port[w, 1].Q_flow - G_w[w, 1, t] * (T[w, 1, t] - T[w - 1, 1, t]) - G_d[2, w, t] * (T[w, 1, t] - T[w, 2, t]);
  C[1, d, t] * der(T[1, d, t]) = G_t[t, 1, d] * (T[1, d, t - 1] - T[1, d, t]) + hp_ext.port[1, d].Q_flow - G_w[2, d, t] * (T[1, d, t] - T[2, d, t]) - G_d[d, 1, t] * (T[1, d, t] - T[1, d - 1, t]);
  C[w, d, t] * der(T[w, d, t]) = G_t[t, w, d] * (T[w, d, t - 1] - T[w, d, t]) + hp_ext.port[w, d].Q_flow - G_w[w, d, t] * (T[w, d, t] - T[w - 1, d, t]) - G_d[d, w, t] * (T[w, d, t] - T[w, d - 1, t]);
...
//"central" lumps along all directions
  ...
  annotation(
    Icon(graphics = {Rectangle(fillColor = {211, 215, 207}, fillPattern = FillPattern.Solid, extent = {{-100, 20}, {100, -20}}), Line(origin = {-59.7867, 0.0234569}, points = {{0, 10}, {0, -10}}), Line(origin = {-59.7867, 0.0234569}, points = {{0, 20}, {0, -20}}), Line(origin = {-40.1038, -0.197413}, points = {{0, 20}, {0, -20}}), Line(origin = {-29.3865, -0.059678}, points = {{0, 20}, {0, -20}}), Line(origin = {-20.1038, 0.0780571}, points = {{0, 20}, {0, -20}}), Line(origin = {6.93102, 0.215792}, points = {{0, 20}, {0, -20}}), Line(origin = {19.6207, -0.00510068}, points = {{0, 20}, {0, -20}}), Line(origin = {59.7454, -0.0466795}, points = {{0, 20}, {0, -20}}), Line(origin = {65.0832, -0.0882583}, points = {{0, 20}, {0, -20}}), Line(origin = {79.7454, 0.0494768}, points = {{0, 20}, {0, -20}}), Line(origin = {0, -3.51767}, points = {{-100, 0}, {100, 0}}), Line(origin = {0.137735, 5.40645}, points = {{-100, 0}, {100, 0}}), Line(origin = {-0.0415792, 2.53743}, points = {{-100, 0}, {100, 0}}), Line(origin = {0.0961562, 12.1788}, points = {{-100, 0}, {100, 0}}), Line(origin = {-0.220893, -13.7801}, points = {{-100, 0}, {100, 0}})}));
