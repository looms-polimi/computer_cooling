...
model Tube_1D
  ComputerCooling.OnePhaseLiquidComponents.Transfer.Ducts.LiquidStream_0D liquidStream(Dstream = D, L = L, dz = dz, w_nom = w_nom, dp_nom = dp_nom, TStart = TStart) annotation(
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a hp(each T(start = TStart)) annotation(
  ...
  ComputerCooling.HeatTransfer.StreamSeparators.TubeWall_1D tubeWall(L = L, W = W, t = t, material = material, TStart = TStart, n = n) annotation(
